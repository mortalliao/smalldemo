package abc.small.demo.zookeeper;
 ...
 *
 * <pre>
 *     所有操作zookeeper的操作都将通过调用ZooKeeper类的方法来完成。
 *     除非另有说明，否则此类的方法是线程安全的。
 *
 *     客户端与服务器建立连接后，会为客户端分配一个会话ID。
 *     客户端将定期向服务器发送心跳以保持会话有效。只要客户端的会话ID保持有效，应用程序就可以通过客户端调用ZooKeeper API。
 *     如果由于某种原因，客户端长时间无法向服务器发送心跳（例如，超过sessionTimeout值），则服务器将使会话到期，并且会话ID将失效。
 *     客户端对象将不再可用。要进行ZooKeeper API调用，应用程序必须创建一个新的客户端对象。
 *
 *     如果由于某种原因，客户端长时间无法向服务器发送心跳（例如，超过sessionTimeout值），则服务器将使会话到期，并且会话ID将失效。
 *     客户端对象将不再可用。要进行ZooKeeper API调用，应用程序必须创建一个新的客户端对象。
 *
 *     ZooKeeper API方法有同步或异步两种。同步方法会阻塞，直到服务器响应。异步方法只是将发送请求排队并立即返回。
 *     它们采用一个回调对象，该回调对象将在成功执行请求时执行，或者在错误时执行，并返回指示错误的返回代码。
 *
 *     一些成功的ZooKeeper API调用可以将监视(Watcher)留在ZooKeeper服务器中的“数据节点”上。
 *     其他成功的ZooKeeper API调用可以触发这些Watcher。
 *     一旦Watcher被触发，事件将被传递给客户端，
 *     假如Watcher触发之后,立刻get节点信息,客户端在得到 Watch 消息之前肯定不可能看到更新后的数据。
 *     换句话说，更新通知先于更新结果。
 *
 *     Watch 只会被触发一次。如果客户端想得到后续更新的通知，必须要在 Watch 被触发后重新注册一个 Watch。
 *
 *     当客户端丢失当前连接后重新连接服务器，所有被认为是触发的监视器，但是没有送达的事件将丢失。
 *     为了模式这个场景，客户端将产生一个特殊的事件，去告诉事件处理器连接被删除。
 *     这个特殊的事件的类型是EventNone 状态是KeeperStateDiscounnected。
 *
 * </pre>
 */
public class TestZookeeper {
    ...
    public static void main(String[] args) throws IOException, InterruptedException {
        ...
        getData(zooKeeper);
        setData(zooKeeper);
        deteleNode(zooKeeper);
        nodeExists(zooKeeper);
        acl(zooKeeper);
    }

    public static ZooKeeper createSession() throws InterruptedException {
        //===============================创建会话======================================
        ...
        try {
            zookeeper1 = new ZooKeeper("127.0.0.1:2181", 5000, new MyWatcher());
        } catch (IOException e) {
            e.printStackTrace();
        }
        ...
        try {
            zookeeper2 = new ZooKeeper("127.0.0.1:2181", 5000, new MyWatcher(), sessionId, sessionPasswd);
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    public static void createNode(ZooKeeper zookeeper1) throws InterruptedException {
        //===============================创建节点======================================
         ...
         */
        try {
            zookeeper1.create("/node", "123".getBytes(),
         ...
         */
        try {
            zookeeper1.create("/node1", "123".getBytes(),
    ...
    public static void getData(ZooKeeper zookeeper1) throws InterruptedException {
        //===============================获取节点数据======================================
        /**
         * 每一步操作/node3都要先执行该语句
        ...
        try {
            data = zookeeper1.getData("/node3", new MyWatcher(), new Stat());
        } catch (KeeperException | InterruptedException e) {
            e.printStackTrace();
        }
    ...
    public static void setData(ZooKeeper zookeeper1) throws InterruptedException {
        //===============================修改节点数据======================================

        /**
         * setData节点有version这一参数，给定版本与节点的版本匹配，则设置给定路径的节点的数据（如果给定版本是-1，则它匹配任何节点的版本）。
         * 返回节点的统计信息。
         *
        ...
        Stat stat = null;
        try {
            stat = zookeeper1.setData("/node3", "嗨喽".getBytes(), -1);
    ...
    public static void deteleNode(ZooKeeper zookeeper1) throws InterruptedException {
        //===============================删除节点======================================
    ...
    public static void nodeExists(ZooKeeper zookeeper1) {
        //===============================判断节点是否存在======================================
        ...
        Stat exists = null;
        try {
            exists = zookeeper1.exists("/node2", new Watcher() {
                @Override
    ...
    public static void acl(ZooKeeper zookeeper1) {
        ...
        Stat auth = null;
        try {
            auth = zookeeper1.setACL("/node3", Collections.singletonList(new ACL(ZooDefs.Perms.ALL, new Id("auth", "helloworld:123456"))), -1);
        } catch (KeeperException | InterruptedException e) {
            e.printStackTrace();
        }
        ...
        List<ACL> acl = null;
        try {
            acl = zookeeper1.getACL("/node3", new Stat());
        } catch (KeeperException | InterruptedException e) {
            e.printStackTrace();
        }
